...
abstract: "Practitioner models and experimentation suggest that Peer-to-Peer lending
  ...
  quality data analysis. The existing body of research and models are generally focused on understanding the profitiability 
  by mimimizing the probability of default rather than optimizing for profitability. 
  To encourage the consolidation of knowledge, I've prepared a resource bibliography
  of research and articles from around the web that could be used to understand
  the current state of analysis.\n\nArticles can be classified into one
  of three categories based on their rigor and depth of analysis.  This bibliogrpahy puts forward the 
  categorization of- simplistic, exploratory, and explanatory. Simplistic models are early entries
  into the topic but do not provide significant insight.  Exploratory models give a methodical review of 
  loan attributes with hypothesis for further testing.  Explanantory articles seek to explain a narrowly 
...
An article proposing a decision support system for choosing loans to optimize profitability.  The previous literature was focused on modeling default probabilities without considering the increased interest rates to compensate the investor's risk.  The authors proposed using internal rate of return or IRR to meausre of a loan's prfitability.  IRR was chosen because it is able to handle uneven payments at non uniform intervals.  They created several models predict a loans IRR as the dependent variable. 
