...
An expansion on the author's earlier analysis but with discussion around challenges in improving the model.  One such challenge is overcoming the numberous unique employment titles to create meaningful groupings.
...
A blog post which quickly moves through a cursory exploratory analysis and on to applying machine learning algorithms.  O'Rourke applies two algorithms to Lending Club's historical loan data from 2007-2011: Decision Tree and Logistic Regression.  The author did not develope a hypothesis to test nor contributed new insights to the body of knowledge. The article is a common form of analysis meant to demonstrate the author's ability to apply a model to a dataset (a necessary step in the learning process).
